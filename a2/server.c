...
void decodePacket(char* buffer, struct packet* result){
	...
	for(int i = 0;;i++){
		...
		//	printf("%d",objectNumber);
			memcpy(temp, buffer+startFrom, sectionSize);
			...
			if(objectNumber == 1){
				...
				strcpy(result->filename, temp);
				result->filedata = (char*)malloc(sizeof(char)*(result->size+1));
				memcpy(result->filedata, buffer + startFrom, sizeof(char) * (result->size+1));
				//memcpy(result->filedata, buffer+startFrom);
